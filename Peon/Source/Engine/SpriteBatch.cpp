...
void SpriteBatch::AddSprite(const glm::vec3& position, const glm::vec3& rotation, const glm::vec3& scale, grim::Sprite* sprite)
{
    grim::Texture* texture = sprite->spriteSheet;
    grim::ShaderProgram* shaderProgram = sprite->shaderProgram;
    ...
    uint16_t col = std::floor(sprite->frame / spriteSheetCols);
    ...
    rotationMatrix = glm::rotate(rotationMatrix, glm::radians(rotation.x), glm::vec3(1.0, 0.0, 0.0));
    ...
    glm::vec4 pos(position, 0.0);
    glm::vec4 topLeft = (glm::vec4(-(scale.x / 2), -(scale.y / 2), 0.0, 1.0) * rotationMatrix) + pos;
    glm::vec4 topRight = (glm::vec4((scale.x / 2), -(scale.y / 2), 0.0, 1.0) * rotationMatrix) + pos;
    glm::vec4 bottomLeft = (glm::vec4(-(scale.x / 2), (scale.y / 2), 0.0, 1.0) * rotationMatrix) + pos;
    glm::vec4 bottomRight = (glm::vec4((scale.x / 2), (scale.y / 2), 0.0, 1.0) * rotationMatrix) + pos;

    // Vertices
    AddVertex(grim::Vertex(glm::vec3(topLeft.x, topLeft.y, topLeft.z), sprite->color, glm::vec2(spriteTexelX, spriteTexelY)));
    AddVertex(grim::Vertex(glm::vec3(topRight.x, topRight.y, topRight.z), sprite->color, glm::vec2(spriteTexelX + spriteTexelWidth, spriteTexelY)));
    AddVertex(grim::Vertex(glm::vec3(bottomLeft.x, bottomLeft.y, bottomLeft.z), sprite->color, glm::vec2(spriteTexelX, spriteTexelY + spriteTexelHeight)));
    AddVertex(grim::Vertex(glm::vec3(bottomRight.x, bottomRight.y, bottomRight.z), sprite->color, glm::vec2(spriteTexelX + spriteTexelWidth, spriteTexelY + spriteTexelHeight)));
...
void SpriteBatch::AddVertex(const grim::Vertex& vertex)
{
...
void SpriteBatch::Upload()
{
    ...
    {
        vertices[0 + (i * 9)] = m_vertexDataBuffer[i].x;
        vertices[1 + (i * 9)] = m_vertexDataBuffer[i].y;
        vertices[2 + (i * 9)] = m_vertexDataBuffer[i].z;
        vertices[3 + (i * 9)] = m_vertexDataBuffer[i].r;
        vertices[4 + (i * 9)] = m_vertexDataBuffer[i].g;
        vertices[5 + (i * 9)] = m_vertexDataBuffer[i].b;
        vertices[6 + (i * 9)] = m_vertexDataBuffer[i].a;
        vertices[7 + (i * 9)] = m_vertexDataBuffer[i].u;
        vertices[8 + (i * 9)] = m_vertexDataBuffer[i].v;
    }
